...
def lineplot(vertices,indices,h=[],alpha=1.,labels=False,linewidths=1):
    """
    ...
    if len(h) > 0:
        color = cm.ScalarMappable(Normalize(h.min(), h.max()), cm.rainbow)
        # alpha = (alpha - alpha.min()) / (alpha.max() - alpha.min())
        # c = numpy.vstack([numpy.zeros((3, len(alpha))), alpha ** 2]).T
        c = numpy.array([color.to_rgba(c) for c in h])
        if isinstance(alpha, Iterable):
            c = numpy.vstack([c.T, alpha]).T
        else:
            col.set_alpha(alpha)
        col.set_edgecolors(c)
    else:
        col.set_color('k')
        col.set_linewidth(linewidths)
